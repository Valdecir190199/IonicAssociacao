...
let ListarMateriaPageModule = class ListarMateriaPageModule {
};
...
let ListarMateriaPage = class ListarMateriaPage {
    constructor(fire) {
        this.fire = fire;
        this.listaMateria = this.fire.list('materia').snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(lista => lista.map(linha => (Object.assign({ key: linha.payload.key }, linha.payload.val())))));
    }
    excluir(key, materia) {
    ...
    buscar() {
        if (this.busca != null) {
            this.listaMateria = this.fire.list('materia', ref => ref.equalTo(this.busca)).snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(lista => lista.map(linha => (Object.assign({ key: linha.payload.key }, linha.payload.val())))));
        }
        else {
            this.listaMateria = this.fire.list('materia').snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(lista => lista.map(linha => (Object.assign({ key: linha.payload.key }, linha.payload.val())))));
        }
...
ListarMateriaPage.ctorParameters = () => [
...
//# sourceMappingURL=materia-listar-materia-listar-materia-module-es2015.js.map